...
public static class Challenge01{
    public static string GetResult(){
        try{
            ...
            {
                var word=wordsCounter.FirstOrDefault(x=>x.Key.ToString().Equals(item,StringComparison.OrdinalIgnoreCase));
                if(word.Key is not null) wordsCounter[word.Key]=word.Value+1;
                else wordsCounter.Add(item,1);
            } 
